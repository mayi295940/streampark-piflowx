...
public class XMLBuilder implements Serializable {
    ...
    public static Map<String, String> getNodeByXPath(
                                                     Document document, Map<String, String> schema_xpathExpressions) {
        Map<String, String> nodeText = new HashMap<>(schema_xpathExpressions.size());
        NodeList nodeList;
        ...
        for (Map.Entry<String, String> schema_xpathExpression : schema_xpathExpressions.entrySet()) {
            try {
                nodeList =
                    (NodeList) xpath.evaluate(schema_xpathExpression.getValue(), document, XPathConstants.NODESET);
                contents = new ArrayList<>(nodeList.getLength());
                for (int i = 0; i < nodeList.getLength(); i++) {
    ...
    public static void main(String[] args) {
        ...
        List<String> institutes =
            XMLBuilder.getNodeByXPath(document, "/articles/article/authorlist/author/institute");
    }
}
