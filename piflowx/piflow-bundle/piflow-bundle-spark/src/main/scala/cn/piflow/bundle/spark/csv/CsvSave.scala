...
class CsvSave extends ConfigurableStop[Null, DataFrame, Null] {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val partition = new PropertyDescriptor()
      ...
      .description(
        "The partition of csv file,you can specify the number of partitions saved as csv or not")
      .defaultValue("")
      .required(false)
      .example("3")
  ...
  override def perform(
    ...
    if ("".equals(partition)) {
      ...
      df.repartition(partition.toInt)
        .write
        .format("csv")
        .mode(saveMode)
        .option("header", header)
