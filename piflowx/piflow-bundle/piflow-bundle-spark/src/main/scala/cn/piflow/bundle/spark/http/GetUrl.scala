...
class GetUrl extends ConfigurableStop[DataFrame] {
  ...
  override def perform(
    ...
    if (httpAcceptTypes == "xml") {
      ...
      val arrbuffer: ArrayBuffer[Element] = ArrayBuffer()
      arrbuffer += rootElt
      ...
      for (x <- (1 until arrLabel.length)) {
        ...
        if (x == 1) {
          ele = rootElt.element(arrLabel(x))
        } else {
          ele = arrbuffer(x - 2).element(arrLabel(x))
        }
        arrbuffer += ele
      }
      ...
      val scalaIterator: Iterator[Element] =
        FatherInterator.asInstanceOf[util.Iterator[Element]].asScala
      ...
      val listRows: List[Row] = list.toList.map(
        line => {
      ...
      val fields: Array[StructField] = arrSchame.map(
        p => {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val url = new PropertyDescriptor()
      ...
      .required(true)
      .example(
        "https://api.elsevier.com/content/search/scopus?query=TITLE('title')&apiKey=555637gxd\"")
