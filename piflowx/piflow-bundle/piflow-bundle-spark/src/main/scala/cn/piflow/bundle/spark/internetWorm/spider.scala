...
class spider extends ConfigurableStop[DataFrame] {
  ...
  override def perform(
    ...
    try {
      ...
      case e: Exception =>
        throw new RuntimeException(
          "The page specified by firstUrl does not exist." +
    ...
    try {
      ...
      case e: Exception =>
        throw new RuntimeException(
          "JumpDependence specified label path does not exist." +
  ...
  private def parseNext(url: String, strArr: Array[String]): Unit = {
    var doc: Document = null
    try {
      ...
      case e: IllegalArgumentException =>
        throw new RuntimeException(
          "The two level interface path does not exist." +
    ...
    for (each <- strArr) {
      ...
      if (fileMapArr(0).indexOf("DOWN") > -1) {
        ...
        for (r <- (rangeArr(0).toInt until rangeArr(1).toInt)) {
          ...
          val eachFile: File = new File(
            downPath + map.get(markupField).get + Constants.SINGLE_SLASH + eachFileEle.text())
          if (!eachFile.exists()) {
            val in: InputStream = new URL(url + eachFileEle.attr("href")).openStream()
            val out: BufferedOutputStream =
              new BufferedOutputStream(new FileOutputStream(eachFile), 10485760)
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val rootUrl = new PropertyDescriptor()
      .name("rootUrl")
      .displayName("rootUrl")
      .description("The domain name address of Web site that needs to crawl data.")
      .defaultValue("")
      .required(true)
    descriptor = rootUrl :: descriptor

    val markupField = new PropertyDescriptor()
      .name("markupField")
      .displayName("markupField")
      .description("The field name of the collection obtained from the first level interface.")
      .defaultValue("")
      .required(true)
    descriptor = markupField :: descriptor

    val firstUrl = new PropertyDescriptor()
      .name("firstUrl")
      .displayName("firstUrl")
      .description("Address of the first level interface of website")
      .defaultValue("")
      .required(true)
    descriptor = firstUrl :: descriptor

    val jumpDependence = new PropertyDescriptor()
      .name("jumpDependence")
      .displayName("jumpDependence")
      .description("Href access to the second tier interface")
      .defaultValue("")
      .required(true)
    ...
    val fileMap = new PropertyDescriptor()
      .name("fileMap")
      .displayName("fileMap")
      .description("The fields and label paths you want, and the location of the data, " +
        "are separated by /, and different fields are connected by'+'. If you want to specify a link to the download list, " +
        "you can do so.Number of Instances/html>body>table>tbody>tr>td>table>tbody>tr>td>p.normal/4+Data Set Characteristics/html>body>table>tbody>tr>td>table>tbody>tr>td>p.normal/2+DOWN/html>body>table>tbody>tr>td>table>tbody>tr>td>p>span.normal>a/0/html>body>table>tbody>tr>td>a/1-2")
      .defaultValue("")
      .required(true)
    descriptor = fileMap :: descriptor

    val downPath = new PropertyDescriptor()
      .name("downPath")
      .displayName("downPath")
      .description("The path of the dataset you want to save")
      .defaultValue("")
      .required(false)
