...
class DecisionTreeTraining extends ConfigurableStop[Null, DataFrame, Null] {
  ...
  def perform(
    ...
    // Maximum number of bins used for discretizing continuous
    // features and for choosing how to split on features at each node.
    ...
    val maxBins = new PropertyDescriptor()
      ...
      .description(
        "Maximum number of bins used for discretizing " +
          "continuous features and for choosing how to " +
          "split on features at each node.")
      .defaultValue("")
      .required(true)
