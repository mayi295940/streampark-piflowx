...
class GBTTraining extends ConfigurableStop[Null, DataFrame, Null] {
  ...
  def perform(
    ...
    // More bins give higher granularity.Must be >= 2 and >= number of categories
    ...
    // Param for the name of family which is a description of the label
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val maxBins = new PropertyDescriptor()
      .name("maxBins")
      .displayName("MAX_BINS")
      .description("Maximum number of bins used for discretizing " +
        "continuous features and for choosing how to split on features at each node.")
      .defaultValue("")
      .required(false)

    val maxDepth = new PropertyDescriptor()
      .name("maxDepth")
    ...
    val minInfoGain = new PropertyDescriptor()
      .name("minInfoGain")
      .displayName("MIN_INFO_GAIN")
      .description("Minimum information gain for a split " +
        "to be considered at a tree node")
      .defaultValue("")
      .required(false)

    val minInstancesPerNode = new PropertyDescriptor()
      .name("minInstancesPerNode")
      .displayName("MIN_INSTANCES_PER_NODE")
      .description("Minimum number of instances each child must have " +
        "after split.")
      .defaultValue("")
      .required(false)

    val impurity = new PropertyDescriptor()
      .name("impurity")
      .displayName("IMPURITY")
      .description("Criterion used for information gain calculation " +
        "(case-insensitive). Supported: \"entropy\" and \"gini\". (default = gini)")
      .defaultValue("")
      .required(false)

    val subSamplingRate = new PropertyDescriptor()
      .name("subSamplingRate")
      .displayName("SUB_SAMPLING_RATE")
      .description("Fraction of the training data used for learning " +
        "each decision tree, in range (0, 1].")
      .defaultValue("")
      .required(false)

    val lossType = new PropertyDescriptor()
      .name("lossType")
      .displayName("LOSS_TYPE")
      .description("Loss function which GBT tries to minimize. " +
        "(case-insensitive) Supported: \"logistic\" (default = logistic)")
      .defaultValue("")
      .required(false)

    val stepSize = new PropertyDescriptor()
      .name("stepSize")
      .displayName("STEP_SIZE")
      .description("Param for Step size (a.k.a. learning rate) in " +
        "interval (0, 1] for shrinking the contribution of each estimator. (default = 0.1)")
      .defaultValue("")
      .required(false)
