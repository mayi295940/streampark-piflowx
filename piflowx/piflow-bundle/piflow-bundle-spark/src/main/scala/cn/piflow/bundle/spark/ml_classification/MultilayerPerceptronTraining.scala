...
class MultilayerPerceptronTraining extends ConfigurableStop[DataFrame] {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val minTol = new PropertyDescriptor()
      .name("minTol")
      .displayName("MIN_TOL")
      .description("Param for the convergence tolerance for " +
    ...
    val stepSize = new PropertyDescriptor()
      ...
      .description("Param for Step size to be used for each iteration " +
        "of optimization (> 0).")
      .defaultValue("")
      .required(false)
    ...
    val thresholds = new PropertyDescriptor()
      ...
      .description(
        "DoubleArrayParam.Param for Thresholds in multi" +
          "class classification to adjust the probability of predicting each class. " +
          ...
          "largest value p/t is predicted, where p is the original probability of " +
          "that class and t is the class's threshold.")
      .defaultValue("")
      .required(true)

    val layers = new PropertyDescriptor()
      .name("layers")
      .displayName("LAYERS")
      .description("Layer sizes including input size and output size. ")
      .defaultValue("")
      .required(true)
