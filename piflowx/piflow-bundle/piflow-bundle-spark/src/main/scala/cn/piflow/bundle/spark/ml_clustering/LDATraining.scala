...
class LDATraining extends ConfigurableStop[DataFrame] {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val docConcentration = new PropertyDescriptor()
      .name("docConcentration")
      .displayName("DOC_CONCENTRATION")
      .description("Concentration parameter (commonly named \"alpha\") " +
        "for the prior placed on documents' distributions over topics (\"theta\").")
      .defaultValue("")
      .required(false)

    val topicConcentration = new PropertyDescriptor()
      .name("topicConcentration")
      .displayName("TOPIC_CONCENTRATION")
      .description(
        "Concentration parameter (commonly named \"beta\" or \"eta\") for the prior placed on topics' distributions over terms.")
      .defaultValue("")
      .required(false)

    val checkpointInterval = new PropertyDescriptor()
      .name("checkpointInterval")
      .displayName("CHECK_POINT_INTERVAL")
      .description(
        "Param for set checkpoint interval (>= 1) " +
          "or disable checkpoint (-1). E.g. 10 means that the cache will get checkpointed " +
          "every 10 iterations.")
      .defaultValue("")
      .required(true)

    val optimizer = new PropertyDescriptor()
      .name("optimizer")
      .displayName("OPTIMIZER")
      .description("Optimizer or inference algorithm used to estimate the LDA model.")
      .defaultValue("")
      .required(false)
