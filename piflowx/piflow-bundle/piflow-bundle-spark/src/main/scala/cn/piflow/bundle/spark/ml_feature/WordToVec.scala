package cn.piflow.bundle.spark.ml_feature
...
class WordToVec extends ConfigurableStop[DataFrame] {
  ...
  def perform(
    ...
    // Any sentence longer than this threshold will be
    // divided into chunks of up to maxSentenceLength size. Default: 1000
    ...
    // included in the word2vec model's vocabulary. Default: 5
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val vectorSize = new PropertyDescriptor()
      .name("vectorSize")
      .displayName("VECTOR_SIZE")
      .description("The dimension of the code that you want to transform from words. Default: 100")
      .defaultValue("")
      .required(false)

    val maxSentenceLength = new PropertyDescriptor()
      .name("maxSentenceLength")
      .displayName("MAX_SENTENCE_LENGTH")
      .description("Sets the maximum length (in words) of each sentence in the input data. " +
        "Any sentence longer than this threshold will be divided into chunks of up to maxSentenceLength size. Default: 1000")
      .defaultValue("")
      .required(false)
    ...
    val minCount = new PropertyDescriptor()
      .name("minCount")
      .displayName("MIN_COUNT")
      .description("The minimum number of times a token must appear to be" +
        " included in the word2vec model's vocabulary. Default: 5")
      .defaultValue("")
      .required(false)
    ...
    val numPartitions = new PropertyDescriptor()
      .name("numPartitions")
      .displayName("NUM_PARTITIONS")
      .description("Param for Step size to be used for " +
        "each iteration of optimization (> 0).")
      .defaultValue("")
      .required(false)

    val colName = new PropertyDescriptor()
      .name("colName")
      .displayName("INPUT_COL")
      .description("")
      .defaultValue("")
      .required(true)
