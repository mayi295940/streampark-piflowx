...
class RdfToDF extends ConfigurableStop[DataFrame] {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val propertyRegex = new PropertyDescriptor()
      ...
      .description(
        "define the propertyRegex to parse the n3 file's property lines\r\n" +
      ...
      .defaultValue(
        "<(?<prefix>http:\\/\\/[^>]+\\/)(?<id>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> <(?<pprefix>http:\\/\\/[^>]+\\/)(?<name>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> \"(?<value>.+)\" \\.")
      .required(true)
    ...
    val relationshipRegex = new PropertyDescriptor()
      ...
      .description(
        "define the propertyRegex to parse the n3 file's relationship lines\r\n" +
          "this regex string should be fully named and regular\r\n" +
          ...
          "check the default value carefully to knowledge the right structure")
      .defaultValue(
        "<(?<prefix1>http:\\/\\/[^>]+\\/)(?<id1>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> <(?<tprefix>http:\\/\\/[^>]+\\/)(?<type>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)(?<!#type)> <(?<prefix2>http:\\/\\/[^>]+\\/)(?<id2>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> \\.")
      .required(true)
    ...
    val entityRegex = new PropertyDescriptor()
      ...
      .description(
        "define the propertyRegex to parse the n3 file's entity lines\r\n" +
          "this regex string should be fully named and regular\r\n" +
          ...
          "check the default value carefully to knowledge the right structure")
      .defaultValue(
        "(<(?<prefix>http:\\/\\/[^>]+\\/)(?<id>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> <(?:http:\\/\\/[^>]+\\/)(?:[^\\/][-A-Za-z0-9._#$%^&*!@~]+)(?:#type)> <(?<lprefix>http:\\/\\/[^>]+\\/)(?<label>[^\\/][-A-Za-z0-9._#$%^&*!@~]+)> \\.")
      .required(true)
    ...
    val relationshipSchema = new PropertyDescriptor()
      ...
      .description(
        "define the schema of relationship, as a user, \r\n" +
    ...
    val entityIdName = new PropertyDescriptor()
      ...
      .description(
        "define the id of entity, as a user, \r\n" +
    ...
    val entityLabelName = new PropertyDescriptor()
      ...
      .description(
        "define the label of entity, as a user, \r\n" +
  ...
  override def perform(
    ...
    if (isFront == "true") {
      ...
      while (iterator.hasNext) { // every row's first col should be the exact hdfs path of the n3 file
        index match {
          case 0 =>
            hdfsFile = sc.textFile(iterator.next())
            index += 1
          case 1000 =>
            println(
              hdfsFile.count()
            ) // in some case the num of file will excess 10w, use this way to reduce the depth of DAG
            index = 1
          case _ =>
            hdfsFile = hdfsFile.union(sc.textFile(iterator.next))
            index += 1
        }
      }
    } else {
    ...
    val entityDFSList: Seq[StructField] = for {
      ...
      p match {
        ...
        case _: Array[String] =>
          StructField(s, DataTypes.createArrayType(StringType), nullable = true)
        case _ => StructField(s, StringType, nullable = true)
      }
    }

    val relationshipDFSchema: StructType = StructType(
      RSchema
        .split(",")
        .map(x => x.trim)
  ...
  private def get(m: Matcher, name: String): String = {
    if (m.find()) m.group(name)
    else ""
  }

  private def ?(regex: Pattern, str: String): Boolean = {
