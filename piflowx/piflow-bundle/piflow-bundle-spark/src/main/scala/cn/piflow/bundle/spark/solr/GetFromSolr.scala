...
class GetFromSolr extends ConfigurableStop[DataFrame] {
  ...
  override def perform(
    ...
    val resultsList: SolrDocumentList = response.getResults
    val listbuffer: ListBuffer[Map[String, AnyRef]] = new ListBuffer[Map[String, AnyRef]]
    var keySTR: String = ""
    ...
    val rows1: List[Row] = listbuffer.toList.map(
      map => {
  ...
  override def getPropertyDescriptor(): List[PropertyDescriptor] = {
    ...
    val indent = new PropertyDescriptor()
      ...
      .displayName("Indent")
      .description(
        "Whether the returned result is indented.It is disabled by default.It is enabled with indent = ture|on")
      .defaultValue("")
      .required(false)
      .example("ture|on")
