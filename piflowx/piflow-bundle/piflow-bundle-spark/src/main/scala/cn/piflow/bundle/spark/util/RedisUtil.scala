...
object RedisUtil extends Serializable {
  ...
  def manipulateRow(
      row: Row,
      column_name: String,
      jedisClusterImplSer: JedisClusterImplSer): Unit = {
  ...
  def putRedis(
      ...
      build: String,
      valueIndex: Int,
      jedisCluster: JedisCluster): Unit = {
  ...
  def checkRedis(
      ...
      tableName: String,
      psnType: String,
      checkField: Seq[String],
      jedisCluster: JedisCluster): String = {
    var Psn = ""
    var hasPSN = false
    var hasNull = false
    breakable {
  ...
  def recordMKDup(
      rows: Iterable[(String, Row)],
      tableName: String,
      jedisCluster: JedisCluster): (String, Row) = {
  ...
  def getMKFather(
    ...
    if (s == null || s == key)
      jedisCluster.hget(tableName + "@" + key, psnType + "IdPSN") // not son of any return itself
  ...
  def gatherDup(rows: Array[Row], primaryKeyIndex: Int, primaryKey: String): Row = {
    ...
    for (index <- 1 until rows.length) {
      ...
      if (row != null && f != null) { // null pointer
        father =
          gatherDup_(father, row.toSeq.toArray, onlyNullReplace = true, primaryKeyIndex, primaryKey)
      }
    }
