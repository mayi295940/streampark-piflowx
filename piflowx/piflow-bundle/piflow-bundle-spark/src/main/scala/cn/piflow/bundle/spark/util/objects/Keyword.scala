...
/**
 * (1) 如果候选关键词在往年都没有被用户使用过（完全新的关键词）， 如果关键词在研究方向下出现了2次以上，加权频次在2.0以上， 在研究方向下出现的比例大于0.7（研究方向下加权频次 ÷
 * 学部下加权频次）， 将关键词补充到研究方向下。
 *
 * (2) 如果候选关键词在往年被填写过（但没有被专家采用）， 如果关键词在研究方向下出现了15次以上，55次以下， 加权频次在6.0以上，在研究方向下出现的比例大于0.5，
 * 将关键词补充到研究方向下。(需要历年的推荐数据)
 *
 * @param name
 * @param iterable
 */
class Keyword(val name: String, iterable: Iterable[(Hierarchy, Int, Boolean)])
    ...
    for (pair <- hiMap) {
      hiMap = hiMap.updated(
        pair._1, {
          ...
          new KeywordStatus(
            pair._2.keywordFrequency,
            StringUtil.countString(absTitle._1, name),
