...
public enum DataBaseType {
  ...
  SQLSSERVER_2017_LATER(
      "sqlserver_2017_later", "com.microsoft.sqlserver.jdbc.SQLServerDriver", "select 1111"),
  ...
  public static DataBaseType getByTypeName(String typeName) {
    if (StringUtils.isEmpty(typeName)) {
      DataBaseType[] values = DataBaseType.values();
      for (DataBaseType dataBaseType : values) {
        if (dataBaseType.getTypeName().equalsIgnoreCase(typeName)) {
          return dataBaseType;
        }
      }
    }
