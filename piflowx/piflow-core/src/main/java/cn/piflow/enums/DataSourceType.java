...
/** @author ：mayi 数据源类型 值 1000 以上表示未启用，后续标号 99 以下倒序的表示定制化的需求 */
public enum DataSourceType {

    // RDBMS
    MySQL(1, 0, "MySQL", "MySQL", "mysql5"),
    MySQL8(1001, 1, "MySQL", "MySQL8", "mysql8"),
    ...
    DataSourceType(
                   int val, int order, String name, String groupTag, String pluginName, String driverClassName) {
        this.val = val;
        this.order = order;
        this.name = name;
        this.groupTag = groupTag;
    ...
    public static final Map<String, List<Integer>> GROUP =
        new HashMap<>(DataSourceType.values().length);
    ...
    public static DataSourceType getSourceType(int value) {
        ...
        throw new RuntimeException(String.format("Data source type[%s] is not supported", value));
    }
     ...
     * @return 数据源类型
     */
    public static DataSourceType getSourceTypeByPluginName(String pluginName) {
        ...
        throw new RuntimeException(
            String.format("Data source plugin[%s] is not supported", pluginName));
    }
    ...
    public static List<String> getRDBMSNameList() {
        List<Integer> rdbms = getRDBMS();
        List<String> rdbmsNameList = new ArrayList<>();
        for (DataSourceType type : DataSourceType.values()) {
            if (rdbms.contains(type.val)) {
                rdbmsNameList.add(type.name.toLowerCase());
            }
        }
        return rdbmsNameList;
    }

    public static boolean isRDBMS(String dataSourceType) {
        List<Integer> rdbms = getRDBMS();
        for (DataSourceType type : DataSourceType.values()) {
            if (type.getName().equalsIgnoreCase(dataSourceType) && rdbms.contains(type.val)) {
                return true;
            }
        }
        return false;
