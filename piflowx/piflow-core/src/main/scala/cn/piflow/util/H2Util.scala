...
object H2Util {
  ...
  private val CREATE_THOUGHPUT_TABLE =
    "create table if not exists thoughput (flowId varchar(255), " +
      "stopName varchar(255), portName varchar(255), count long)"

  private val CREATE_FLAG_TABLE =
    "create table if not exists configFlag(id bigint auto_increment, " +
      "item varchar(255), flag int, createTime varchar(255))"

  private val CREATE_SCHEDULE_TABLE =
    "create table if not exists schedule(id bigint auto_increment, " +
  ...
  def getFlowProgressPercent(appId: String): String = {
    ...
    val completedRS: ResultSet = statement.executeQuery(
      "select count(*) as completedStopCount from stop where flowId='" + appId + "' and state='" + StopState.COMPLETED + "'")
  ...
  def getStartedStop(appId: String): List[String] = {
    ...
    val rs: ResultSet = statement.executeQuery(
      "select * from stop where flowId='" + appId + "' and state = '" + StopState.STARTED + "'")
    while (rs.next()) {
  ...
  def getGroupState(groupId: String): String = {
    ...
    val groupRS: ResultSet =
      statement.executeQuery("select state from flowGroup where id='" + groupId + "'")
    if (groupRS.next()) {
  ...
  def getGroupChildByStatus(groupId: String, status: String): List[String] = {
    ...
    val groupRS: ResultSet =
      statement.executeQuery("select * from flowGroup where parentId='" + groupId + "'")
    breakable {
      while (groupRS.next()) {
  ...
  def getGroupInfoMap(groupId: String): Map[String, Any] = {
    ...
    val flowGroupRS: ResultSet =
      statement.executeQuery("select * from flowGroup where id='" + groupId + "'")
    ...
    val flowRS: ResultSet =
      statement.executeQuery("select * from flow where groupId='" + groupId + "'")
    while (flowRS.next()) {
  ...
  def getGroupProgressPercent(groupId: String): String = {
    ...
    if (groupState.equals(FlowState.COMPLETED)) {
      ...
      val completedGroupRS: ResultSet = statement.executeQuery(
        "select count(*) as completedGroupCount from flowGroup where parentId='" + groupId + "' and state='" + GroupState.COMPLETED + "'")
      ...
      val completedFlowRS: ResultSet = statement.executeQuery(
        "select count(*) as completedFlowCount from flow where GroupId='" + groupId + "' and state='" + FlowState.COMPLETED + "'")
  ...
  def getFlag(item: String): Int = {
    ...
    val flowGroupRS: ResultSet =
      statement.executeQuery("select flag from configFlag where item='" + item + "'")
    if (flowGroupRS.next()) {
  ...
  def addScheduleInstance(
    ...
    statement.executeUpdate(
      "insert into scheduleInstance(id, cronExpression, startDate, endDate, state, createTime, updateTime) values('" + scheduleId + "','" + cronExpression + "','" + startDate + "','" + endDate + "','" + state + "','" + time + "','" + time + "')")
    statement.close()
  }
  ...
  def updateScheduleInstanceStatus(scheduleId: String, state: String) = {
    ...
    val time = new Date().toString
    val updateSql =
      "update scheduleInstance set state='" + state + "', updateTime='" + time + "' where id='" + scheduleId + "'"
    println(updateSql)
    statement.executeUpdate(updateSql)
    statement.close()
  }
  ...
  def getScheduleInfo(scheduleId: String): String = {
    ...
    // get flow basic info
    val scheduleInstanceRS: ResultSet =
      statement.executeQuery("select * from scheduleInstance where id='" + scheduleId + "'")
    ...
    val scheduleRS: ResultSet =
      statement.executeQuery("select * from schedule where scheduleId='" + scheduleId + "'")
    while (scheduleRS.next()) {
  ...
  def getStartedSchedule(): List[String] = {
    ...
    var scheduleInfoMap = Map[String, Any]()
    // get flow basic info
    val scheduleInstanceRS: ResultSet = statement.executeQuery(
      "select * from scheduleInstance where state='" + ScheduleState.STARTED + "'")
    while (scheduleInstanceRS.next()) {
  ...
  def addPlugin(name: String) = {
    ...
    if (!rs.isBeforeFirst) {
      ...
      val time = new Date().toString
      statement.executeUpdate(
        "insert into plugin(id, name, state, createTime, updateTime) values('" + id + "','" + name + "','" + PluginState.ON + "','" + time + "','" + time + "')")
      ...
      breakable {
        while (rs.next()) {
          ...
          if (state == PluginState.OFF) {
            val updateSql =
              "update plugin set state='" + PluginState.ON + "'," + "updateTime='" + time + "' where name='" + name + "'"
            statement.executeUpdate(updateSql)
            state = PluginState.ON
            break
          }
      ...
      breakable {
        while (rs.next()) {
          ...
          if (state == PluginState.ON) {
            val updateSql =
              "update plugin set state='" + PluginState.OFF + "'," + "updateTime='" + time + "' where name='" + name + "'"
            statement.executeUpdate(updateSql)
      ...
      breakable {
        while (rs.next()) {
          ...
          if (state == SparkJarState.OFF) {
            val updateSql =
              "update sparkJar set state='" + SparkJarState.ON + "'," + "updateTime='" + time + "' where name='" + sparkJarName + "'"
            statement.executeUpdate(updateSql)
            state = SparkJarState.ON
            break
          }
      ...
      breakable {
        while (rs.next()) {
          ...
          if (state == SparkJarState.ON) {
            val updateSql =
              "update sparkJar set state='" + SparkJarState.OFF + "'," + "updateTime='" + time + "' where id='" + sparkJarId + "'"
            statement.executeUpdate(updateSql)
  ...
  def getSparkJarOn(): List[String] = {
    ...
    val statement = getConnectionInstance().createStatement()
    val rs: ResultSet =
      statement.executeQuery("select * from sparkJar where state='" + SparkJarState.ON + "'")
    while (rs.next()) {
      pluginList = rs.getString("name") +: pluginList
    }
    rs.close()
