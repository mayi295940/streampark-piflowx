...
object API {
  ...
  def getResourceInfo: String = {
    ...
    try {
      ...
      val remainingVirtualCores =
        totalVirtualCores.asInstanceOf[Double] - allocatedVirtualCores.asInstanceOf[Double]
      val cpuInfo = Map(
  ...
  def getFlowCheckpoint(appId: String): String = {
    val checkpointPath = ConfigureUtil
      .getCheckpointPath()
      .stripSuffix(Constants.SINGLE_SLASH) + Constants.SINGLE_SLASH + appId
  ...
  def getFlowDebugData(appId: String, stopName: String, port: String): String = {
    ...
    val debugPath: String = ConfigureUtil
      .getDebugPath()
      .stripSuffix(
        Constants.SINGLE_SLASH) + Constants.SINGLE_SLASH + appId + Constants.SINGLE_SLASH + stopName + Constants.SINGLE_SLASH + port
    val schema = HdfsUtil.getLine(debugPath + "_schema")
    val result = "{\"schema\":\"" + schema + "\", \"debugDataPath\": \"" + debugPath + "\"}"
    result
  }

  def getFlowVisualizationData(
      appId: String,
      stopName: String,
  ...
  private def getLogFile(uuid: String, appName: String): (File, File) = {
    ...
    val stdoutPathString = PropertyUtil.getPropertyValue(
      "log.path") + Constants.SINGLE_SLASH + appName + "_" + uuid + "_stdout_" + nowDate
    val stdout = new File(stdoutPathString)

    val stderrPathString = PropertyUtil.getPropertyValue(
      "log.path") + Constants.SINGLE_SLASH + appName + "_" + uuid + "_stderr_" + nowDate
