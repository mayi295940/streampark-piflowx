...
public class PaginationInterceptor implements InnerInterceptor {
    ...
    protected static final List<SelectItem> COUNT_SELECT_ITEM = Collections.singletonList(
        new SelectExpressionItem(new Column().withColumnName("COUNT(1)")).withAlias(new Alias("total")));
    ...
    protected MappedStatement buildAutoCountMappedStatement(MappedStatement ms) {
        ...
        return CollectionUtils.computeIfAbsent(countMsCache, countId, key -> {
            MappedStatement.Builder builder =
                new MappedStatement.Builder(configuration, key, ms.getSqlSource(), ms.getSqlCommandType());
