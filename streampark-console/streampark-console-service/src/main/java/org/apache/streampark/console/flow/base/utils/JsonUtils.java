...
public class JsonUtils {
  ...
  /**
   * When converting "JSON" when using "Jackson", the date format setting "Jackson" two methods set
   * the date format of the output
   *
   * <p>1. Ordinary way: The default is to convert to "timestamps" form, you can cancel "timestamps"
   * by the following way.
   * objectMapper.configure(SerializationConfig.Feature.WRITE_DATES_AS_TIMESTAMPS, false); This will
   * cause the time generation to use the so-called use a [ISO-8601]-compliant notation, which
   * outputs a time similar to the following: "1970-01-01T00:00:00.000+0000". Of course, you can
   * also customize the output format:
   * objectMapper.getSerializationConfig().setDateFormat(myDateFormat); The myDateFormat object is
   * java.text.DateFormat, which uses the annotation method of checking java API 2.annotaion: First
   * define the format you need as follows
   *
   * <p>Then find the date get method on your POJO @JsonSerialize(using =
   * CustomDateSerializer.class) public Date getCreateAt() { return createAt; }
   *
   * <p>"java" date object converted to "JSON" date formatted custom class via "Jackson" library
   *
   * @date 2010-5-3
   */
  ...
  public static <T> T readJson(JsonNode node, Class<?> parametrized, Class<?>... parameterClasses)
      throws Exception {
    JavaType javaType =
