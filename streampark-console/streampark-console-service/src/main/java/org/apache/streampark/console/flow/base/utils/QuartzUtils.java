...
public class QuartzUtils {
  ...
  public static void createScheduleJob(Scheduler scheduler, SysSchedule sysSchedule) {
    try {
      ...
      // The timing task class needs to be a concrete implementation of the job class. QuartzJobBean
      // is an abstract class for jobs.
      Class<? extends Job> jobClass =
          (Class<? extends Job>) Class.forName(sysSchedule.getJobClass());
      // Build timed task information
      JobDetail jobDetail =
          JobBuilder.newJob(jobClass).withIdentity(sysSchedule.getJobName()).build();
      // Set the timing task execution mode
      CronScheduleBuilder scheduleBuilder =
          CronScheduleBuilder.cronSchedule(sysSchedule.getCronExpression());
      // Build trigger trigger
      CronTrigger trigger =
          TriggerBuilder.newTrigger()
              .withIdentity(sysSchedule.getJobName())
