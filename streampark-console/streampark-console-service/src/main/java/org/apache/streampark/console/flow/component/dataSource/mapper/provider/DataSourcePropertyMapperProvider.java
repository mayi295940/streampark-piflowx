...
public class DataSourcePropertyMapperProvider {
  ...
  private void preventSQLInjectionDataSourceProperty(DataSourceProperty dataSourceProperty) {
    if (null != dataSourceProperty
        && StringUtils.isNotBlank(dataSourceProperty.getLastUpdateUser())) {
      // Mandatory Field
      ...
      String dataSourceIdStr =
          (null != dataSourceProperty.getDataSource()
              ? dataSourceProperty.getDataSource().getId()
              : null);
      this.dataSourceId =
   ...
   *
   * @param map (Content: dataSourcePropertyList, the value is
   *     List<datasourceproperty></datasourceproperty>)
   * @return
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
  public String addDataSourcePropertyList(Map map) {
    List<DataSourceProperty> dataSourcePropertyList =
        (List<DataSourceProperty>) map.get("dataSourcePropertyList");
