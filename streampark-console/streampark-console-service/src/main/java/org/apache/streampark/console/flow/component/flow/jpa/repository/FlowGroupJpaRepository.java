...
public interface FlowGroupJpaRepository
    extends JpaRepository<FlowGroup, String>, JpaSpecificationExecutor<FlowGroup>, Serializable {

  @Query("select c from  FlowGroup c where c.enableFlag=:enableFlag and c.id=:id")
  FlowGroup getFlowGroupByIdAndEnAndEnableFlag(
      @Param("id") String id, @Param("enableFlag") boolean enableFlag);

  @Query(
      value =
          "select name from  flow_group c where c.enable_flag=:enableFlag and  c.name=:name and c.fk_flow_group_id=:flowGroupId",
      nativeQuery = true)
   ...
   */
  @Query(
      "select c from FlowGroup c where c.enableFlag=true and c.isExample<>true and c.flowGroup is null and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
   ...
   */
  @Query(
      "select c from FlowGroup c where c.enableFlag=true and c.isExample<>true and c.flowGroup is null and c.crtUser=:userName and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
  Page<FlowGroup> getFlowGroupListPageByCrtUser(
