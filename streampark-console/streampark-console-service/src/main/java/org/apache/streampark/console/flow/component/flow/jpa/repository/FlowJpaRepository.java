...
public interface FlowJpaRepository
    extends JpaRepository<Flow, String>, JpaSpecificationExecutor<Flow>, Serializable {
   ...
   */
  @Query(
      "select c from Flow c where c.enableFlag=true and c.isExample<>true and c.flowGroup is null and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
   ...
   */
  @Query(
      "select c from Flow c where c.enableFlag=true and c.isExample<>true and c.flowGroup is null and c.crtUser=:userName and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
  Page<Flow> getFlowListPage(
  ...
  @Query(
      nativeQuery = true,
      value =
          "select * from flow s where s.enable_flag=1 and s.fk_flow_group_id=:fid and s.page_id=:pageId")
  ...
  @Query(
      nativeQuery = true,
      value =
          "select MAX(s.page_id+0) from flow s where s.enable_flag=1 and s.fk_flow_group_id=:flowGroupId")
  ...
  String[] getFlowNameByFlowGroupId(
