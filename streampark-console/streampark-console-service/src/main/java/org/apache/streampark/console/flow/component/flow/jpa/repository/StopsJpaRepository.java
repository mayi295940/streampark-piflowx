...
public interface StopsJpaRepository
    extends JpaRepository<Stops, String>, JpaSpecificationExecutor<Stops>, Serializable {
  ...
  @Query(
      nativeQuery = true,
      value = "select MAX(page_id+0) from flow_stops where enable_flag=1 and fk_flow_id=:flowId ")
  Integer getMaxStopPageIdByFlowId(@Param("flowId") String flowId);

  @Query(
      nativeQuery = true,
      value = "SELECT fs.name from flow_stops fs WHERE fs.enable_flag=1 and fs.fk_flow_id =:flowId")
