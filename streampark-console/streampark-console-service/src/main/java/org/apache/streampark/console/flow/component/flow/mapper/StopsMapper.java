...
public interface StopsMapper {
  ...
  @Select(
      "SELECT name FROM flow_stops WHERE enable_flag = 1 and fk_data_source_id = #{datasourceId}")
  ...
  @Select(
      "SELECT page_id FROM flow_stops "
          + "WHERE enable_flag = 1 "
          + "and is_disabled = 1 "
  ...
  @Results({
    ...
    @Result(
        column = "id",
        property = "stopsCustomizedPropertyVoList",
        many =
            @Many(
                select =
  ...
  @Select(
      "SELECT fs.id, fs.name FROM flow_stops fs\n"
   ...
   */
  @Select(
      "SELECT COUNT(fs.id) AS num FROM flow_stops fs "
          + "WHERE fs.enable_flag = 1 "
          + "AND fs.fk_flow_id = #{flowId};")
