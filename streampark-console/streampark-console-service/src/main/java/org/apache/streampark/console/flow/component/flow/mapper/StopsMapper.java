...
public interface StopsMapper {
  ...
  public List<Stops> getStopsListByFlowIdAndPageIds(
  ...
  @Select(
      "select * from flow_stops fs where fs.fk_flow_id = #{flowId} and fs.page_id = #{stopPageId} and fs.enable_flag = 1 ")
  public Stops getStopByFlowIdAndStopPageId(
   ...
   */
  @Select(
      "select id from flow_stops where name = #{stopName} and fk_flow_id =#{flowId} and enable_flag = 1 ")
  public String getStopByNameAndFlowId(
      @Param(value = "flowId") String flowId, @Param(value = "stopName") String stopName);
  ...
  @Select(
      "select * from flow_stops where enable_flag=1 and fk_flow_id=#{fid} and page_id=#{stopPageId}  limit 1")
  @Results({
    @Result(id = true, column = "id", property = "id"),
    @Result(
        column = "id",
  ...
  public List<String> getStopsNamesByDatasourceId(
      @Param(value = "datasourceId") String datasourceId);
