...
public class FlowGroupPathsMapperProvider {
  ...
  private boolean preventSQLInjectionFlowGroupPaths(FlowGroupPaths flowGroupPaths) {
    ...
    String lastUpdateDttmStr =
        StringUtils.isBlank(flowGroupPaths.getLastUpdateDttmString())
            ? DateUtils.dateTimesToStr(new Date())
            : flowGroupPaths.getLastUpdateDttmString();
    ...
    String flowGroupId =
        null != flowGroupPaths.getFlowGroup() ? flowGroupPaths.getFlowGroup().getId() : null;
