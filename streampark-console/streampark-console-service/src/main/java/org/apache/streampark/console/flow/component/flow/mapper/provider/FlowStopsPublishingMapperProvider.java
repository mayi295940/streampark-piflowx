...
public class FlowStopsPublishingMapperProvider {
  ...
  private boolean preventSQLInjectionFlowStopsPublishing(FlowStopsPublishing flowStopsPublishing) {
    if (null == flowStopsPublishing
        || StringUtils.isBlank(flowStopsPublishing.getLastUpdateUser())
        || StringUtils.isBlank(flowStopsPublishing.getPublishingId())) {
      return false;
    }
    if (null == flowStopsPublishing.getStopsIds()
        || flowStopsPublishing.getStopsIds().size() <= 0) {
      return false;
    }
    // Mandatory Field
  ...
  public String updateFlowStopsPublishingEnableFlagByPublishingId(
  ...
  public String updateFlowStopsPublishingEnableFlagByPublishingIdAndStopId(
  ...
  public static String getFlowStopsPublishingList(String username, boolean isAdmin, String param) {
    ...
    // LEFT JOIN flow_stops_publishing fsp2 ON fsp1.publishing_id = fsp2.publishing_id AND fsp1.id >
    // fsp2.id
    // LEFT JOIN flow_stops fs ON fsp1.stops_id=fs.id
    // WHERE fsp2.publishing_id IS NULL AND fsp1.enable_flag=1 AND fs.enable_flag=1 AND
    // fsp1.crt_user='admin'
   ...
   * @return
   */
  public String getFlowStopsPublishingByPublishingIdAndCreateUser(
      String username, String publishingId) {
    if (StringUtils.isBlank(publishingId)) {
      return "SELECT 0";
    }
   ...
   * @return
   */
  public String getFlowStopsPublishingListByPublishingIdAndStopsId(
  ...
  public String getFlowStopsPublishingListByFlowId(String username, String flowId) {
    ...
    // WHERE fsp.enable_flag=1 AND fs.enable_flag=1 AND fsp.crt_user='admin' AND
    // fs.fk_flow_id='6111a00006a44a1e87d112f289d54640';
    StringBuffer sql = new StringBuffer();
    sql.append(
        "SELECT DISTINCT fsp.publishing_id, fsp.name FROM flow_stops_publishing fsp LEFT JOIN flow_stops fs ON fsp.stops_id=fs.id ");
  ...
  public String getPublishingNameListByFlowId(String flowId) {
    ...
    // AND fsp.stops_id IN (SELECT fs.id FROM flow_stops fs WHERE fs.is_disabled=0 AND fs.fk_flow_id
    // ='71213b942bee4c5994775774f45ff5ad' )
    ...
    sqlBuffer.append(
        " SELECT fs.id FROM flow_stops fs WHERE fs.is_disabled=0 AND fs.fk_flow_id = ");
