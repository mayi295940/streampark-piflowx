...
package org.apache.streampark.console.flow.component.livy.mapper.provider;
...
public class CodeSnippetMapperProvider {
    ...
    private boolean preventSQLInjectionCodeSnippet(CodeSnippet codeSnippet) {
        ...
        String lastUpdateDttm =
            DateUtils.dateTimesToStr(
                null != codeSnippet.getLastUpdateDttm() ? codeSnippet.getLastUpdateDttm() : new Date());
        this.id = SqlUtils.preventSQLInjection(codeSnippet.getId());
        this.enableFlag =
            ((null != codeSnippet.getEnableFlag() && codeSnippet.getEnableFlag()) ? 1 : 0);
        ...
        String noteBookIdStr =
            (null != codeSnippet.getNoteBook() ? codeSnippet.getNoteBook().getId() : null);
    ...
    public String getCodeSnippetList(boolean isAdmin, String username, String param) {
        ...
        if (StringUtils.isNotBlank(param)) {
            ...
            stringBuf.append(
                "description LIKE CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%')");
        }
