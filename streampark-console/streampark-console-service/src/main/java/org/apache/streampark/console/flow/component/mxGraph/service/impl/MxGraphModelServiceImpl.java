...
import org.apache.streampark.console.flow.base.util.FlowXmlUtils;
...
import org.apache.streampark.console.flow.component.flow.jpa.domain.FlowGroupDomain;
...
import org.apache.streampark.console.flow.component.mxGraph.jpa.domain.MxCellDomain;
import org.apache.streampark.console.flow.component.mxGraph.jpa.domain.MxGeometryDomain;
import org.apache.streampark.console.flow.component.mxGraph.jpa.domain.MxGraphModelDomain;
...
public class MxGraphModelServiceImpl implements IMxGraphModelService {
  ...
  @Resource private MxGraphModelDomain mxGraphModelDomain;

  @Resource private MxCellDomain mxCellDomain;

  @Resource private MxGeometryDomain mxGeometryDomain;
  ...
  public String saveDataForTask(String username, String imageXML, String loadId, String operType) {
    ...
    if ("ADD".equals(operType)) {
      ...
      return JsonUtils.toJsonNoException(
          this.addFlowStops(username, xmlToMxGraphModel, loadId, true));
    } else if ("MOVED".equals(operType)) {
      logger.info("MOVED Operation begins");
      return JsonUtils.toJsonNoException(
          this.updateFlowMxGraph(username, xmlToMxGraphModel, loadId));
   ...
   * @return map
   */
  private Map<String, Object> addFlowStops(
      String username, MxGraphModelVo mxGraphModelVo, String flowId, boolean flag) {
    ...
    for (MxCellVo mxCellVo : addMxCellVoList) {
      if (null != mxCellVo) {
        String stopByNameAndFlowId =
            stopsMapper.getStopByNameAndFlowId(flow.getId(), mxCellVo.getValue());
        // save MxCell
        // new
    ...
    Map<String, List<MxCellVo>> stopsPathsMap =
        MxGraphModelUtils.distinguishElementsPaths(addMxCellVoList);
    ...
    List<Paths> addPathsList =
        MxGraphModelUtils.mxCellVoListToPathsList(username, objectPaths, flow);
    if (flag) {
      // Judge empty
      if (null != addPathsList && addPathsList.size() > 0) {
        ...
        if (addPathsListNum <= 0) {
          logger.error(
              "addPathsList Save failed flow：flowId(" + flow.getId() + ")",
              new Exception("addPathsList save failed"));
        }
      } else {
   ...
   * @return
   */
  private Map<String, Object> updateFlowMxGraph(
      String username, MxGraphModelVo mxGraphModelVo, String flowId) {
    if (StringUtils.isBlank(username)) {
   ...
   * @return
   */
  private Map<String, Object> updateFlow(
      String username, MxGraphModelVo mxGraphModelVo, String flowId) {
    if (StringUtils.isBlank(username)) {
      logger.warn("illegal user");
      return ReturnMapUtils.setFailedMsg("illegal user");
    ...
    Map<String, Object> updateMxGraphRtnMap =
        this.updateFlowMxGraph(username, mxGraphModelVo, flowId);
    // Determine if mxGraphModel is saved successfully
    if (null == updateMxGraphRtnMap
        || 200 != (int) updateMxGraphRtnMap.get(ReturnMapUtils.KEY_CODE)) {
    ...
    Map<String, List<MxCellVo>> stopsPathsMap =
        MxGraphModelUtils.distinguishElementsPaths(mxCellVoList);
    ...
    // Determine whether the list of lines in the database is empty, do not empty to continue the
    // judgment operation below, or directly add
    if (null != pathsList && pathsList.size() > 0) {
      ...
      for (Paths paths : pathsList) {
        if (null != paths) {
          ...
          if (null != mxCellVo) {
            // There is no operation to modify the stop information when you operate the drawing
            // board. You can modify the properties, but save it immediately. No operation is
            // required here.
          } else {
    ...
    // If the list of Stops in the database is empty, the modification fails because this method
    // only processes the modification and is not responsible for adding
    if (null != stopsList && stopsList.size() > 0) {
      ...
      // The data stopsList of the loop database, using the pageId in the stops to convert to the
      // map of the value of the page after the map is fetched,
      for (Stops stops : stopsList) {
        if (null != stops) {
          ...
          // If you get it, you need to modify it. Otherwise, it is to be deleted.
          if (null != mxCellVo) {
            // When you operate the drawing board, when you delete the line, you need to determine
            // whether there is an Any type stop at both ends of the line. If there is any
            // corresponding port information in the attribute, delete the corresponding port
            // information.
            ...
            if (stops.getInPortType() == PortType.ANY || stops.getOutPortType() == PortType.ANY) {
              ...
              // If inprot or outprot has a value, the loop attribute finds the attribute of the
              // corresponding storage port and modifies it.
  ...
  private void addStopList(List<Stops> addStopsList) {
    ...
    for (Stops stops : addStopsList) {
      ...
      if (addPropertyList <= 0) {
        logger.error(
            "Property save failed：stopsId("
                + stops.getId()
                + "),stopsName("
                + stops.getName()
                + ")",
            new Exception("Property save failed"));
      }
    }
  }
   ...
   * @return
   */
  private List<Stops> mxCellVoListToStopsList(
      List<MxCellVo> objectStops, Flow flow, String username) {
    ...
    if (null != objectStops && objectStops.size() > 0) {
      ...
      for (MxCellVo mxCellVo : objectStops) {
        ...
        if (null != stops) {
          String stopByNameAndFlowId =
              stopsMapper.getStopByNameAndFlowId(flow.getId(), stops.getName());
          if (StringUtils.isNotBlank(stopByNameAndFlowId)) {
  ...
  private StopsComponent getStopsTemplate(String stopsName) {
    ...
    List<StopsComponent> stopsComponentList =
        stopsComponentMapper.getStopsComponentByName(stopsName);
   ...
   * @return
   */
  private Map<String, Object> updateMxCellList(
      String username, List<MxCellVo> mxCellVoList, List<MxCell> mxCellList) {
    if (StringUtils.isBlank(username)) {
      logger.warn("illegal user");
      return ReturnMapUtils.setFailedMsg("illegal user");
    }
    ...
    if (null != updateMxCellList && updateMxCellList.size() > 0) {
      for (MxCell mxCell : updateMxCellList) {
        ...
        if (updateMxCell <= 0) {
          logger.warn(
              "mxGeometryVo save failed：pageId("
                  + mxCell.getPageId()
                  + "),name("
                  + mxCell.getValue()
                  + ")",
              new Exception("mxGeometryVo save failed"));
          continue;
        }
        MxGeometry mxGeometry = mxCell.getMxGeometry();
        if (null == mxGeometry) {
        ...
        if (updateMxGeometry > 0) {
          ...
          logger.error(
              "mxGeometryVo save failed：The corresponding mxCell's pageId("
                  + mxCell.getPageId()
                  + "),mxCellValue("
                  + mxCell.getValue()
                  + ")",
  ...
  private Map<String, Object> addGroupFlows(
      String username, MxGraphModelVo mxGraphModelVo, String flowGroupId) {
    if (StringUtils.isBlank(username)) {
    ...
    if (null == flowGroup) {
      return ReturnMapUtils.setFailedMsg(
          "No query to flowId is: " + flowGroupId + " Flow information");
    }
  ...
  private Map<String, Object> updateFlowGroup(
    ...
    if (null != elementsAndPathsMap) {
      ...
      Map<String, MxCellVo> stopsMxCellVoMap =
          stopsMxCellVoListToMap(elementsAndPathsMap.get("elements"));
   ...
   * @return
   */
  private Map<String, Object> updateGroupMxCellList(
      List<MxCellVo> mxCellVoList, List<MxCell> mxCellList, String username) {
    // If the mxCellList is empty, the modification fails because this method only processes the
    // modifications and is not responsible for adding
    if (null == mxCellList || mxCellList.size() <= 0) {
      return ReturnMapUtils.setFailedMsg(
  ...
  public String addMxCellAndData(MxGraphVo mxGraphVo, String username) {
    ...
    for (MxCellVo mxCellVo : addMxCellVoList) {
      ...
      String flowGroupIdByPageId =
          flowGroupMapper.getFlowGroupIdByPageId(loadId, mxCellVo.getPageId());
  ...
  private FlowGroup addFlowGroupNodeAndEdge(
    ...
    Map<String, List<MxCellVo>> flowGroupNodeAndEdge =
        MxGraphModelUtils.distinguishElementsPaths(addMxCellVo);
