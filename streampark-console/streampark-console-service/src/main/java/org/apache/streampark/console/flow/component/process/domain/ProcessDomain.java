...
public class ProcessDomain {
  ...
  public ProcessDomain(
      ProcessStopCustomizedPropertyMapper processStopCustomizedPropertyMapper,
  ...
  public int addProcess(Process process) throws Exception {
    ...
    String[] globalParamsIds =
        FlowGlobalParamsUtils.globalParamsToIds(process.getFlowGlobalParamsList());
  ...
  public int addProcessStopCustomizedProperty(
      ProcessStopCustomizedProperty processStopCustomizedProperty) throws Exception {
    if (null == processStopCustomizedProperty) {
      throw new Exception("save failed");
    }
    int affectedRows =
        processStopCustomizedPropertyMapper.addProcessStopCustomizedProperty(
  ...
  public int updateProcessStopPropertyList(List<ProcessStopProperty> processStopPropertyList)
      throws Exception {
    if (null == processStopPropertyList || processStopPropertyList.size() <= 0) {
      return 0;
    }
  ...
  public boolean updateProcessEnableFlag(String username, boolean isAdmin, String processId) {
    ...
    if (null != processPathList && processPathList.size() > 0) {
      int updateEnableFlagByProcessId =
          processPathMapper.updateEnableFlagByProcessId(processId, username);
    ...
    if (null != processStopList && processStopList.size() > 0) {
      for (ProcessStop processStop : processStopList) {
        if (null != processStop) {
          ...
          if (null != processStopPropertyList && processStopPropertyList.size() > 0) {
            int updateEnableFlagByProcessStopId =
                processStopPropertyMapper.updateEnableFlagByProcessStopId(
                    processStop.getId(), username);
            affectedLine += updateEnableFlagByProcessStopId;
          }
        }
      }
      int updateEnableFlagByProcessId =
          processStopMapper.updateEnableFlagByProcessId(processId, username);
      affectedLine += updateEnableFlagByProcessId;
    }
  ...
  public Process getProcessByPageId(
