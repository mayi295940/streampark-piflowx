...
public class ProcessDomainU {
  ...
  @Resource private ProcessStopPropertyMapper processStopPropertyMapper;
  ...
  public int addProcess(Process process) throws Exception {
      ...
      for (ProcessStop processStop : processStopList) {
        ...
        if (null != processStopPropertyList && processStopPropertyList.size() > 0) {
          for (ProcessStopProperty processStopProperty : processStopPropertyList) {
            ...
            int addProcessStopProperty =
                processStopPropertyMapper.addProcessStopProperty(processStopProperty);
    ...
    int influenceCounts =
        (addProcessPathCounts
            + addProcessStopCounts
            + addProcessStopPropertyCounts
            + addMxGraphModel);
        ...
        for (ProcessStop processStop : processStopList) {
          if (null != processStop) {
            ...
            if (null != processStopPropertyList && processStopPropertyList.size() > 0) {
              for (ProcessStopProperty processStopProperty : processStopPropertyList) {
                if (null != processStopProperty) {
                  int updateProcessStopProperty =
                      processStopPropertyMapper.updateProcessStopProperty(processStopProperty);
