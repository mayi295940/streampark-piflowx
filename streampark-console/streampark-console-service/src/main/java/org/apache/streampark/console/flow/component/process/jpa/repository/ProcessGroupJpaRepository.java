...
public interface ProcessGroupJpaRepository
    extends JpaRepository<ProcessGroup, String>,
        JpaSpecificationExecutor<ProcessGroup>,
        Serializable {
  ...
  @Query(
      value =
          "select c from ProcessGroup c where c.enableFlag=true and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
  ...
  @Query(
      value =
          "select c from ProcessGroup c where c.enableFlag=true and c.crtUser=:userName and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
  Page<ProcessGroup> getProcessGroupListPageByUser(
  ...
  @Modifying
  @Query(
      "update ProcessGroup c set c.enableFlag = :enableFlag, c.lastUpdateUser = :lastUpdateUser, c.lastUpdateDttm = :lastUpdateDttm where c.id = :id")
  int updateEnableFlagById(
      @Param("id") String id,
      @Param("lastUpdateUser") String lastUpdateUser,
      @Param("lastUpdateDttm") Date lastUpdateDttm,
  ...
  @Query(
      value =
          "select app_id from flow_process_group where enable_flag=1 and app_id is not null and ( (state!='COMPLETED' and state!='FAILED' and state!='KILLED') or state is null )",
      nativeQuery = true)
  List<String> getRunningProcessGroupAppId();

  @Query(
      value =
          "SELECT re.name AS name,re.page_id AS pageId FROM ( "
  ...
  List<Map<String, Object>> getProcessGroupNamesAndPageIdsByPageIds(
      @Param("fid") String fid, @Param("pageIds") List<String> pageIds);
  ...
  @Query(
      value =
          "SELECT id,last_update_dttm AS lastUpdateDttm,crt_dttm AS crtDttm,app_id AS appId,name,description,start_time AS startTime,end_time AS endTime,progress,state,parent_process_id AS parentProcessId,processType FROM ("
      ...
      countQuery =
          "SELECT COUNT(re.id) FROM "
              ...
              + ") AS re",
      nativeQuery = true)
  Page<Map<String, Object>> getProcessAndProcessGroupListPage(
  ...
  @Query(
      ...
      countQuery =
          "SELECT COUNT(re.id) FROM "
              ...
              + ") AS re",
      nativeQuery = true)
  Page<Map<String, Object>> getProcessAndProcessGroupListPageByUser(
      @Param("userName") String userName, @Param("param") String param, Pageable pageable);
}
