...
public interface ProcessStopJpaRepository
    extends JpaRepository<ProcessStop, String>,
        JpaSpecificationExecutor<ProcessStop>,
        Serializable {
  /**
   * Paging query
   *
   ...
   */
  @Query(
      "select c from ProcessStop c where c.enableFlag=true and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
   ...
   */
  @Query(
      "select c from ProcessStop c where c.enableFlag=true and c.crtUser=:userName and (c.name like CONCAT('%',:param,'%') or c.description like CONCAT('%',:param,'%'))")
  Page<ProcessStop> getProcessStopListPage(
