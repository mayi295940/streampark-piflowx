...
public class ProcessStopPropertyMapperProvider {
  ...
  private void preventSQLInjectionProcessStopProperty(ProcessStopProperty processStopProperty) {
    if (null != processStopProperty
        && StringUtils.isNotBlank(processStopProperty.getLastUpdateUser())) {
      // Mandatory Field
      ...
      this.required =
          (null == processStopProperty.getRequired()
              ? null
              : (processStopProperty.getRequired() ? 1 : 0));
      this.sensitive =
          (null == processStopProperty.getSensitive()
              ? null
              : (processStopProperty.getSensitive() ? 1 : 0));
      String processStopIdStr =
          (null != processStopProperty.getProcessStop()
              ? processStopProperty.getProcessStop().getId()
              : null);
      this.processStopId =
          (null != processStopIdStr ? SqlUtils.preventSQLInjection(processStopIdStr) : null);
    }
  }
  ...
  public String addProcessStopProperties(
      Map<String, List<ProcessStopProperty>> processStopPropertyList) {
    String sqlStr = "SELECT 0";
    List<ProcessStopProperty> processStopProperties =
        processStopPropertyList.get("processStopPropertyList");
