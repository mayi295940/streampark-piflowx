...
public class StopsComponentMapperProvider {
  ...
  private boolean preventSQLInjectionStops(StopsComponent stopsComponent) {
    ...
    this.inPortType =
        SqlUtils.preventSQLInjection(
            null != stopsComponent.getInPortType() ? stopsComponent.getInPortType().name() : null);
    this.outports = SqlUtils.preventSQLInjection(stopsComponent.getOutports());
    this.outPortType =
        SqlUtils.preventSQLInjection(
            null != stopsComponent.getOutPortType()
                ? stopsComponent.getOutPortType().name()
                : null);
  ...
  public String getStopsComponentListByGroupId(String groupId) {
    ...
    strBuf.append(
        "AND fstm.stops_groups LIKE CONCAT('%',(SELECT fsg1.group_name FROM flow_stops_groups fsg1 WHERE fsg1.id="
            + SqlUtils.preventSQLInjection(groupId)
            + "),'%') ");
    strBuf.append(")");
