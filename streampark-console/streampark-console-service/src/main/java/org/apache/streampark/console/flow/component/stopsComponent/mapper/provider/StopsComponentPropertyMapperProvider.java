...
public class StopsComponentPropertyMapperProvider {
  ...
  private boolean preventSQLInjectionStopsComponentProperty(
      StopsComponentProperty stopsComponentProperty) {
    if (null == stopsComponentProperty
        || StringUtils.isBlank(stopsComponentProperty.getLastUpdateUser())) {
      return false;
    }
    // Selection field
    this.allowableValues =
        SqlUtils.preventSQLInjection(stopsComponentProperty.getAllowableValues());
    ...
    this.required =
        (null == stopsComponentProperty.getRequired()
            ? 0
            : (stopsComponentProperty.getRequired() ? 1 : 0));
    this.sensitive =
        (null == stopsComponentProperty.getSensitive()
            ? 0
            : (stopsComponentProperty.getSensitive() ? 1 : 0));
    this.propertySort =
        (null != stopsComponentProperty.getPropertySort()
            ? stopsComponentProperty.getPropertySort()
            : 0L);
  ...
  public String insertStopsComponentProperty(Map map) {
    String sqlStr = "SELECT 0";
    List<StopsComponentProperty> stopsComponentPropertyList =
        (List<StopsComponentProperty>) map.get("stopsComponentPropertyList");
