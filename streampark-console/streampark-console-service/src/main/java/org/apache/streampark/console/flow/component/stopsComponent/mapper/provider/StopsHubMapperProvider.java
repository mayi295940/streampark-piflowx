...
public class StopsHubMapperProvider {
  ...
  public String updateStopsHub(StopsHub stopsHub) {
    ...
    if (null != stopsHub) {
      ...
      if (mountId != null) sql.SET("mount_id = " + mountId);
      if (jarName != null) sql.SET("jar_name = " + jarName);
      if (jarUrl != null) sql.SET("jar_url = " + jarUrl);
      if (status != null) sql.SET("status = " + status);
