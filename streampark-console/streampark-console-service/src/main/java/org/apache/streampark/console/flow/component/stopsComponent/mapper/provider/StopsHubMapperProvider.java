...
public class StopsHubMapperProvider {
    ...
    private boolean preventSQLInjectionStopsHub(StopsHub stopsHub) {
        ...
        this.status =
            SqlUtils.preventSQLInjection(
                null != stopsHub.getStatus() ? stopsHub.getStatus().name() : null);
        this.bundles = SqlUtils.preventSQLInjection(stopsHub.getBundles());
        this.isPublishing =
            ((null != stopsHub.getIsPublishing() && stopsHub.getIsPublishing()) ? 1 : 0);
        this.type =
            SqlUtils.preventSQLInjection(null != stopsHub.getType() ? stopsHub.getType().name() : null);
    ...
    /**
     * @Description update stop hub type @Param stopsComponents @Return java.lang.String @Author
     * TY @Date 12:58 2023/4/4
     */
    ...
    // sql.append(" WHEN ").append(stopsHub.getId()).append(" THEN
    // ").append(stopsHub.getType().name());
    // }
    // sql.append(" END,version = CASE id");
    // for (StopsHub stopsHub: scalaStopsHubs) {
    // sql.append(" WHEN ").append(stopsHub.getId()).append(" THEN
    // ").append(stopsHub.getVersion() + 1);
    // }
    // sql.append(" END,last_update_dttm = CASE id");
    // for (StopsHub stopsHub: scalaStopsHubs) {
    // sql.append(" WHEN ").append(stopsHub.getId()).append(" THEN
    // ").append(stopsHub.getLastUpdateDttm());
    // }
    // sql.append(" END WHERE id IN (").append(SqlUtils.strListToStr(stopsHubIds)).append(")
    // and enable_flag = 1");
    // return sql.toString();
    // }
