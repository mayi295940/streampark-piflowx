...
public class SysScheduleMapperProvider {
  ...
  private boolean preventSQLInjectionSysSchedule(SysSchedule sysSchedule) {
    ...
    this.enableFlag =
        ((null != sysSchedule.getEnableFlag() && sysSchedule.getEnableFlag()) ? 1 : 0);
    this.version = (null != sysSchedule.getVersion() ? sysSchedule.getVersion() : 0L);
    String lastUpdateDttm =
        DateUtils.dateTimesToStr(
            null != sysSchedule.getLastUpdateDttm() ? sysSchedule.getLastUpdateDttm() : new Date());
    ...
    this.status =
        SqlUtils.preventSQLInjection(
            null != sysSchedule.getStatus() ? sysSchedule.getStatus().name() : "INIT");
    this.lastRunResult =
        SqlUtils.preventSQLInjection(
            null != sysSchedule.getLastRunResult()
                ? sysSchedule.getLastRunResult().name()
                : "INIT");
  ...
  public String getSysScheduleList(boolean isAdmin, String param) {
    ...
    if (isAdmin) {
      ...
      if (StringUtils.isNotBlank(param)) {
        ...
        sqlStrbuf.append("( ");
        sqlStrbuf.append(
            "job_name like CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%') OR ");
        sqlStrbuf.append(
            "job_class like CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%') OR ");
        sqlStrbuf.append(
            "status like CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%') OR ");
        sqlStrbuf.append(
            "cron_expression like CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%') ");
        sqlStrbuf.append(") ");
      }
      sqlStrbuf.append("ORDER BY crt_dttm asc,last_update_dttm DESC");
