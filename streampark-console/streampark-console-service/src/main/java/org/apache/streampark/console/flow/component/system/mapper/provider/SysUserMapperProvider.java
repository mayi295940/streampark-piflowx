...
public class SysUserMapperProvider {
  ...
  private boolean preventSQLInjectionSysUser(SysUser sysUser) {
    ...
    String lastUpdateDttm =
        DateUtils.dateTimesToStr(
            null != sysUser.getLastUpdateDttm() ? sysUser.getLastUpdateDttm() : new Date());
  ...
  public String getSysUserVoList(boolean isAdmin, String username, String param) {
    ...
    StringBuffer strBuf = new StringBuffer();
    strBuf.append(
        "SELECT id, username, name, age, sex, crt_dttm, status, last_login_ip FROM sys_user WHERE enable_flag=1");
