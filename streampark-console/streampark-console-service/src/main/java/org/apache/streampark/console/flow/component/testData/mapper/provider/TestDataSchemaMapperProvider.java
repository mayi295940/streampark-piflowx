...
public class TestDataSchemaMapperProvider {
    ...
    private boolean preventSQLInjectionTestDataSchema(TestDataSchema testDataSchema) {
        ...
        String lastUpdateDttm =
            DateUtils.dateTimesToStr(
                null != testDataSchema.getLastUpdateDttm()
                    ? testDataSchema.getLastUpdateDttm()
                    : new Date());
        this.id = SqlUtils.preventSQLInjection(testDataSchema.getId());
        this.enableFlag =
            ((null != testDataSchema.getEnableFlag() && testDataSchema.getEnableFlag()) ? 1 : 0);
     ...
     * @return
     */
    public String getTestDataSchemaListByTestDataIdSearch(
        ...
        if (StringUtils.isNotBlank(param)) {
            ...
            stringBuf.append(
                "field_name LIKE CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%')");
            stringBuf.append(
                "field_type LIKE CONCAT('%'," + SqlUtils.preventSQLInjection(param) + ",'%')");
        }
