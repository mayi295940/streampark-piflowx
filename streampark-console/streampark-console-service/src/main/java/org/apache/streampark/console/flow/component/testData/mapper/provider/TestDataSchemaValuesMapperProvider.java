...
public class TestDataSchemaValuesMapperProvider {
  ...
  private boolean preventSQLInjectionTestDataSchemaValues(
      TestDataSchemaValues testDataSchemaValues) {
    if (null == testDataSchemaValues
        || StringUtils.isBlank(testDataSchemaValues.getLastUpdateUser())) {
      return false;
    }
    // Mandatory Field
    String lastUpdateDttm =
        DateUtils.dateTimesToStr(
            null != testDataSchemaValues.getLastUpdateDttm()
                ? testDataSchemaValues.getLastUpdateDttm()
                : new Date());
    this.id = SqlUtils.preventSQLInjection(testDataSchemaValues.getId());
    this.enableFlag =
        ((null != testDataSchemaValues.getEnableFlag() && testDataSchemaValues.getEnableFlag())
            ? 1
            : 0);
    this.version =
        (null != testDataSchemaValues.getVersion() ? testDataSchemaValues.getVersion() : 0L);
    ...
    this.testDataId =
        SqlUtils.preventSQLInjection(
            null != testDataSchemaValues.getTestData()
                ? testDataSchemaValues.getTestData().getId()
                : null);
    this.testDataSchemaId =
        SqlUtils.preventSQLInjection(
            null != testDataSchemaValues.getTestDataSchema()
                ? testDataSchemaValues.getTestDataSchema().getId()
                : null);
   ...
   * @return
   */
  public String delTestDataSchemaValuesByTestDataId(
      boolean isAdmin, String username, String testDataId) {
   ...
   * @return
   */
  public String delTestDataSchemaValuesBySchemaId(
      boolean isAdmin, String username, String schemaId) {
  ...
  public static String getTestDataSchemaValuesCustomList(@SuppressWarnings("rawtypes") Map map) {
    ...
    List<LinkedHashMap<String, String>> fieldNameList =
        (List<LinkedHashMap<String, String>>) map.get("fieldNameList");
