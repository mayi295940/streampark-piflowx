...
CREATE TABLE IF NOT EXISTS `FLOW_PROCESS` (
    ...
    `ENVIRONMENT` TEXT COMMENT 'environment',
    ...
    `PAGE_ID` VARCHAR(255) COMMENT 'page_id',
    `RUN_MODE_TYPE` VARCHAR(255) COMMENT 'Process RunModeType',
    `PROCESS_PARENT_TYPE` VARCHAR(255) COMMENT 'Process parent type',
    `FK_FLOW_PROCESS_GROUP_ID` VARCHAR(40) COMMENT 'fk_flow_process_group_id',
    `FK_GROUP_SCHEDULE_ID` VARCHAR(40)
);
...
CREATE TABLE IF NOT EXISTS `FLOW_PROCESS_STOP_PROPERTY`(
    ...
    `CUSTOM_VALUE` LONGTEXT COMMENT 'Custom value',
    `DESCRIPTION` VARCHAR(1024) COMMENT 'Description',
...
CREATE TABLE IF NOT EXISTS `FLOW_STOPS`(
    ...
    `IS_DISABLED` BIT(1),
    `IS_DATA_SOURCE` bit(1) NULL,
    `FK_DATA_SOURCE_ID` VARCHAR(40) COMMENT 'fk_data_source_id',
    `IS_CUSTOMIZED` bit(1) COMMENT 'is_customized',
    `FK_FLOW_ID` VARCHAR(40) COMMENT 'Foreign key flow id'
);
...
CREATE TABLE IF NOT EXISTS `FLOW_STOPS_PROPERTY`(
    ...
    `IS_LOCKED` BIT COMMENT 'is_locked',
    `EXAMPLE` TEXT COMMENT 'property example',
    `NAME` VARCHAR(255) COMMENT 'name',
    `IS_OLD_DATA` BIT COMMENT 'Has it been updated',
    `PROPERTY_REQUIRED` BIT COMMENT 'Property required',
    `PROPERTY_SENSITIVE` BIT COMMENT 'Property sensitive',
    `PROPERTY_SORT` int COMMENT 'property sort',
...
CREATE TABLE IF NOT EXISTS `FLOW_STOPS_TEMPLATE`(
    ...
    `VISUALIZATION_TYPE` VARCHAR(255) NULL COMMENT 'visualization type',
    `COMPONENT_TYPE` VARCHAR(40) NULL COMMENT 'component type',
    `IS_CUSTOMIZED` BIT COMMENT 'is_customized',
    `IS_DATA_SOURCE` BIT COMMENT 'is data source',
    `IMAGE_URL` VARCHAR(255) NULL COMMENT 'stop image url' ,
    `DOCKER_IMAGES_NAME` VARCHAR(255) NULL COMMENT 'docker image name',
    `STOPS_HUB_ID` VARCHAR(40) NULL COMMENT 'stops_hub id'
);
...
CREATE TABLE IF NOT EXISTS `FLOW_STOPS_PROPERTY_TEMPLATE`(
    ...
    `PROPERTY_REQUIRED` BIT COMMENT 'Property required',
    `PROPERTY_SENSITIVE` BIT COMMENT 'Property sensitive',
    `LANGUAGE` VARCHAR(255) NULL COMMENT 'language',
    `EXAMPLE` TEXT COMMENT 'property example',
    `PROPERTY_SORT` int COMMENT 'property sort',
...
CREATE TABLE IF NOT EXISTS `MX_GRAPH_MODEL`(
   `ID` VARCHAR(40) PRIMARY KEY NOT NULL,
	...
	FK_FLOW_GROUP_ID VARCHAR(40) COMMENT 'fk_flow_group_id'
);
...
CREATE TABLE IF NOT EXISTS `DATA_SOURCE`  (
    ...
    `STOPS_TEMPLATE_BUNDLE` varchar(255) NULL,
    `IS_AVAILABLE` bit(1) NULL COMMENT 'is available',
    `IMAGE_URL` varchar(255) NULL COMMENT 'stop image url'
);
