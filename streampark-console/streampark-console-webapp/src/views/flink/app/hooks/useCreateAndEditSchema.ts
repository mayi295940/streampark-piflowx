...
import { deployModes, k8sRestExposedType, resolveOrder } from '../data';
...
import { AppTypeEnum, DeployMode, JobTypeEnum } from '/@/enums/flinkEnum';
import { isK8sDeployMode } from '../utils';
  ...
  */
  ...
  const getExecutionCluster = (
    ...
    return (unref(flinkClusters) || [])
      .filter((o) => o.deployMode == deployMode)
  ...
  const getFlinkClusterSchemas = computed((): FormSchema[] => {
    return [
      ...
      },
      {
        ...
        component: 'Select',
        render: (param) => renderFlinkCluster(getExecutionCluster(DeployMode.DeployMode), param),
        ifShow: ({ values }) =>
      ...
      },
      {
        ...
        ifShow: ({ values }) => values.deployMode == DeployMode.YARN_SESSION && values?.stepCurrent == 0,
        rules: [
          { required: true, message: t('flink.app.addAppTips.flinkClusterIsRequiredMessage') },
        ],
      },
      {
        ...
        ifShow: ({ values }) => values.deployMode == DeployMode.KUBERNETES_SESSION && values?.stepCurrent == 0,
        rules: [
          { required: true, message: t('flink.app.addAppTips.flinkClusterIsRequiredMessage') },
        ],
      },
      {
        ...
        component: 'Input',
        ifShow: ({ values }) => values.deployMode == DeployMode.KUBERNETES_APPLICATION && values?.stepCurrent == 0,
        render: ({ model, field }) =>
  ...
  async function getJobNameCheck(_rule: RuleObject, value: StoreValue, model: Recordable) {
    ...
    if (model.deployMode == DeployMode.KUBERNETES_APPLICATION) {
  ...
  const getFlinkFormOtherSchemas = computed((): FormSchema[] => {
    ...
    return [
      ...
      },
      {
        ...
        ifShow: ({ values }) =>
          edit?.mode == 'flink' ? true : !isK8sDeployMode(values.deployMode) && values?.stepCurrent == 2,
        component: 'InputNumber',
        componentProps: {
      ...
      },
      {
        ...
        show: ({ values }) => ((edit?.mode == 'flink' ? true : !isK8sDeployMode(values.deployMode))) && values?.stepCurrent == 2,
      },
      ...getConfigSchemas(),
      {
        field: 'totalOptions',
      ...
      },
      {
        ...
        ifShow: ({ values }) =>
          ...
          values.deployMode == DeployMode.YARN_PER_JOB) && values?.stepCurrent == 2,
      ...
      },
      {
        ...
        component: 'Input',
        slot: 'podTemplate',
        ifShow: ({ values }) => values.deployMode == DeployMode.KUBERNETES_APPLICATION && values?.stepCurrent == 2,
      },
      {
  ...
  const getDeployModeSchema = computed((): FormSchema[] => {
    return [
      {
        field: 'deployMode',
        ...
        rules: [
          {
            ...
            validator: async (_rule, value) => {
              if (value === null || value === undefined || value === '') {
                return Promise.reject(t('flink.app.addAppTips.deployModeIsRequiredMessage'));
              } else {
                if (
                  [
